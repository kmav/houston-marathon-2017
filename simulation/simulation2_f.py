...
def main():
	...
	sql = "SELECT * FROM MarathonRunners where dropout=0 and half=0 and minute="+str(minute)
	print sql
	cur.execute(sql)
	...
	# group all runners into 5k segments (one array) (non-cumulative)
	...
	##now simulation data (cumulative)
	simData = [0]*10
	...
	while (newobjective < objective and attempt<7):
		...
		sql = "SELECT * FROM MarathonRunners where dropout=0 and half=0 and minute="+str(minute-10)
		print sql
		cur.execute(sql)
		rows = cur.fetchall()
		print "fetching from database ...   ",
		...
		movementsSign = []
		# increases the amount by which speed will be changed (each iteration is more aggressive)
		for m in movements:
			if m>0:
				# everytime we change it by more and more
		...
		print movementsSign;
		#increase/decrease the speed by movementsSign[segment]
		...
		for t in range(2,11,minuteInterval):
			...
			#Go through the previous 10 minutes (we think minute is the current minute)
			t2 = minute-10+t
	...
	for t in range(2,21,minuteInterval):
		...
		for i in range(len(RunnersSorted)):
			...
			if (runner['started']==1 and runner['finished']==0):
				...
				sql = "UPDATE MarathonRunners SET started="+str(started)+", finished="+str(finished)+", position="+str(position)+" where half=0 and runnerId="+str(runnerId)+" and minute="+str(currentTime)+" ; \n"
				#print sql
				cur.execute(sql)
				con.commit()
